<?php
  ...
  if(!empty($main_image_public_url) && !is_null($main_image_public_url)) {
  
  }
  if(!empty($main_image_public_url) && !is_null($main_image_public_url)) { 
    $main_image_path = image_style_url($style, $main_image_public_url);  
    print $output = '<img src="'.$main_image_path.'">';
  }else{
